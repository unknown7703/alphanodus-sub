...
const applicationSchema = z.object({
  ...
  coverLetter: z.string().max(2000, "Cover letter can't exceed 2000 characters.").optional(),
});
...
export default function ApplicationForm({ jobId }: { jobId: number }) {
  ...
  // This function handles the API submission.
  // It constructs a FormData object because we are uploading a file.
  async function onSubmit(values: z.infer<typeof applicationSchema>) {
    ...
    formData.append('resume', values.resume[0]); // Append the file itself
    if (values.coverLetter) {
      formData.append('coverLetter', values.coverLetter);
    }
    
    try {
      // The backend expects 'multipart/form-data', so we don't set Content-Type header.
      // Axios will set it automatically with the correct boundary when given FormData.
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="flex items-center"><Mail className="w-4 h-4 mr-2" />Email Address</FormLabel>
              <FormControl>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="flex items-center"><Phone className="w-4 h-4 mr-2" />Phone Number</FormLabel>
              <FormControl>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="flex items-center"><FileText className="w-4 h-4 mr-2" />Resume (PDF only)</FormLabel>
              <FormControl>
                {/* We use form.register for file inputs with react-hook-form */}
